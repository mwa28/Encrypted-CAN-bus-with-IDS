...
char ecdh_generate_keys(uint8_t* public, uint8_t* private)
{
  ...
  else
  {
    ...
    gf2point_mul((uint32_t*)public, (uint32_t*)(public + BITVEC_NBYTES), (uint32_t*)private);
      ...
      //(DONE) To change: return public.
      
   // return 1;
      
      char *charPublic = malloc(count);
      if(!charPublic)
          return NULL;
      
      for(int i = 0; i < (BITVEC_NBYTES * 2); ++i)
          charPublic[i] = public[i];
...
char ecdh_shared_secret(const uint8_t* private, const uint8_t* others_pub, uint8_t* output)
{
       ...
       &&  gf2point_on_curve((uint32_t*)others_pub, (uint32_t*)(others_pub + BITVEC_NBYTES)) )
  {
    ...
    //NEW ADDITION
    char *charOutput = malloc(count);
    if(!charOutput)
        return NULL;
      
    for(int i = 0; i < (BITVEC_NBYTES * 2); ++i)
        charOutput[i] = output[i];
    ...
    /*
      ...
      */
      //(DONE) TO Change: Return output[i]
    //return 1;
  }
  else
  {

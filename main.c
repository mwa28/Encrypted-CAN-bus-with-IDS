...
static void ecdh_demo(void)
{
    ...
    if(ecdh_generate_keys(puba, prva)){
        ...
        for (unsigned i = 0; i < (ECC_PUB_KEY_SIZE); ++i)
        {
            printf("%hhu", puba[i]);
        }
        printf("\n");
...
//
//    //To Change: Get seca from the other class where it is calculated.
//
//    /* 4. Bob calculates T = b * P = b * (a * g). */
//    assert(ecdh_shared_secret(prvb, puba, secb));
//
//    //To Change: Get secb from the other class where it is calculated.
//

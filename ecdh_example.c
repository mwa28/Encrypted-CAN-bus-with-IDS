...
static void ecdh_demo(void)
{
  ...
  for (i = 0; i < ECC_PRV_KEY_SIZE; ++i)
  {
    ...
    //NEW ADDITION:
    char *publica;
    publica = ecdh_generate_keys(puba, prva);
  ...
  for (i = 0; i < ECC_PRV_KEY_SIZE; ++i)
  {
    ...
    //NEW ADDITION:
    char *publicb;
    publicb = ecdh_generate_keys(pubb, prvb);
  ...
  //NEW ADDITION
   char *outputa;
   outputa = ecdh_shared_secret(prva, pubb, seca);
    
    
    //(DONE) To Change: Get seca from the other class where it is calculated.
  ...
  //assert(ecdh_shared_secret(prvb, puba, secb));
    ...
    //NEW ADDITION
    char *outputb;
    outputb = ecdh_shared_secret(prvb, puba, secb);
    
    //(DONE) To Change: Get secb from the other class where it is calculated.
